...
def get_alignment_heads_for_hf_model(model_name: str):
    """Infer alignment heads for HuggingFace models based on naming patterns"""
    model_name_lower = model_name.lower()
    if "large-v3-turbo" in model_name_lower or "turbo" in model_name_lower:
        return _ALIGNMENT_HEADS.get("large-v3-turbo")
    elif "large-v3" in model_name_lower:
        return _ALIGNMENT_HEADS.get("large-v3")
    elif "large-v2" in model_name_lower:
        return _ALIGNMENT_HEADS.get("large-v2")
    elif "large-v1" in model_name_lower:
        return _ALIGNMENT_HEADS.get("large-v1")
    elif "large" in model_name_lower:
        return _ALIGNMENT_HEADS.get("large")
    elif "medium" in model_name_lower:
        return _ALIGNMENT_HEADS.get("medium")
    elif "small" in model_name_lower:
        return _ALIGNMENT_HEADS.get("small")
    elif "base" in model_name_lower:
        return _ALIGNMENT_HEADS.get("base")
    elif "tiny" in model_name_lower:
        return _ALIGNMENT_HEADS.get("tiny")
    return None
...
def load_torch_weights_and_config(
    ...
    if name_or_path in _MODELS:
        ...
        alignment_heads = get_alignment_heads_for_hf_model(name_or_path)        
        # Try downloading from HF - this is now the default method
        from huggingface_hub import snapshot_download

        # Use a temporary directory if download_root is not suitable for HF models
        cache_dir = tempfile.mkdtemp() if name_or_path in _MODELS else None
        ...
        name_or_path = snapshot_download(
            ...
            allow_patterns=[
                ...
                "pytorch_model.bin", 
                "model.safetensors",
                "model-*.safetensors",  # Include sharded safetensors files
                "*.txt",
            ],
            token=os.getenv("HF_TOKEN")  # Support for private repos
        )
    else:
        if alignment_heads is None:  # Only set if not already set above
            alignment_heads = get_alignment_heads_for_hf_model(name_or_path)
    ...
    if name_or_path.endswith(".pt"):
        ...
        pt_path = base_path / "pytorch_model.bin"
        safetensors_path = base_path / "model.safetensors"
        safetensors_index_path = base_path / "model.safetensors.index.json"
        
        # Try to load weights in order of preference
        if pt_path.is_file():
            weights = torch.load(pt_path, map_location="cpu")
        elif safetensors_path.is_file():
            try:
                # Try to use safetensors library first
                import safetensors.torch
                weights = safetensors.torch.load_file(safetensors_path)
            except ImportError:
                # Fallback to MLX loading if safetensors not available
                weights = mx.load(str(safetensors_path))
        elif safetensors_index_path.is_file():
            # Handle sharded safetensors files
            try:
                import safetensors.torch
                
                # Load the index file to get the shard mapping
            ...
            raise FileNotFoundError(
                ...
                f"Expected pytorch_model.bin, model.safetensors, or model.safetensors.index.json"
            )
        
        # Load config
        config_path = base_path / "config.json"
        if not config_path.exists():
            raise FileNotFoundError(f"No config.json found in {base_path}")
...
if __name__ == "__main__":
    ...
    if args.upload_name is not None:
        upload_to_hub(mlx_path.as_posix(), args.upload_name, args.torch_name_or_path)
